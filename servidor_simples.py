...
def main():
    ...
    thread1 = Thread(target=start_server, args=(PORT, "Principal"), daemon=True)
    thread1.start()
    threads.append(thread1)
    
    # Servidor hotspot (porta 8081)
    thread2 = Thread(target=start_server, args=(MOBILE_HOTSPOT_PORT, "Hotspot"), daemon=True)
    thread2.start()
    ...
    for ip in local_ips:
        print(f"   http://{ip}:{PORT}/main.html")
        print(f"   http://{ip}:{MOBILE_HOTSPOT_PORT}/main.html (hotspot)")
    print()
